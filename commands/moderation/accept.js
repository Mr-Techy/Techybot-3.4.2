...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    if (!messageID)
      return message.channel.send(`You must state the suggestion ID.`);
    if (isNaN(messageID))
      return message.channel.send(`That is not a valid ID.`);
    if (!acceptReason)
      return message.channel.send(`You must state a reason to accept it.`);
    ...
    try {
      ...
      const user = await message.guild.users.cache.find(
        i => i.name == data.author.name
      );
      user.send(
